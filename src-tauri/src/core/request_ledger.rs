...
impl RequestLedgerManager {
    ...
    pub fn record_live_delta(&mut self, delta: &CanonicalTokenDelta) {
        ...
        entry.live_contributed_input += delta.delta_input_tokens;
        ...
        entry.canonical_input_total += delta.delta_input_tokens;
    ...
    pub fn finalize_authoritative(
        ...
        let target_in = normalized.normalized_context_input_tokens;
        ...
        // Reconcile ALL 5 fields (P0-8)
        let diff_in = target_in as i64 - entry.canonical_input_total as i64;
        ...
        entry.authoritative_final_input = Some(target_in);
        ...
        let old_total = entry.canonical_input_total + entry.canonical_output_total;
        let new_total = target_in + target_out;

        entry.canonical_input_total = target_in;
